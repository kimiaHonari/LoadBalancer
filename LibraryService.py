...
class ClientThread(Thread):
    def __init__(self, ip, port ,conn):
        ...
        self.port = port
        self.conn=conn
        print "[+] New libraryServer socket thread started for " + ip + ":" + str(port)
    ...
    def run(self):
        ...
        while True:
            ...
            print "L : received data:", data
            #close connection if receive exit
            if data == "exit":
                self.conn.close()
                break;
            print "Library : ", listOfStudentNumber
            ...
            if hasNumbers(data) :
                ...
                if list[0]!="L" :
                    self.conn.send("L : " + "Command is not correct")
                    continue
                ...
                if any(list[1] in s for s in listOfStudentNumber):
                    ...
                    mutex.acquire()
                    fo = open("LogL.txt", "a")
                    fo.write(datetime.date.today().ctime()+"   L    "+list[1]+" Login to system"+"\n" )
                    fo.close()
                    mutex.release()
                    #end
                    ...
                    result="L : " + "has been registered before"
                    # send result
                    ...
                    fo.write(datetime.date.today().ctime() + "   L    " + list[1] + " Register to system"+"\n" )
                    fo.close()
                    mutex.release()
                    ...
                    time.sleep(RandomIndex/1000)
                    result="L : " + "registration completed"
                    self.conn.send(result)
                ...
                mutex.acquire()
                fo = open("LogL.txt", "a")
                fo.write(datetime.date.today().ctime() + "   L    " +"Client request: "+ data+"\n" )
                fo.close()
                mutex.release()
                #end
                RandomIndex = randint(0, 1500)
...
TCP_IP = '0.0.0.0'
TCP_PORT = 9000 # listen to port 9000
BUFFER_SIZE = 2000
