...
class ClientThread(Thread):
    def __init__(self, ip, port ,conn):
        ...
        self.port = port
        self.conn=conn
        print "[+] New DormitoryServer socket thread started for " + ip + ":" + str(port)
    ...
    def run(self):
        ...
        while True:
            ...
            print "D : received data:", data
            #close connection if receive exit
            if data == "exit":
                self.conn.close()
                break;
            print "Dormitory : ", listOfStudentNumber
            ...
            if hasNumbers(data) :
                ...
                if list[0]!="D" :
                    self.conn.send("D : " + "Command is not correct")
                    continue
                ...
                if any(list[1] in s for s in listOfStudentNumber):
                    ...
                    mutex.acquire()
                    fo = open("LogD.txt", "a")
                    fo.write(datetime.date.today().ctime()+"   D    "+list[1]+" Login to system"+"\n" )
                    fo.close()
                    mutex.release()
                    #end
                    ...
                    result="D : " + "has been registered before"
                    # send result
                    ...
                    fo.write(datetime.date.today().ctime() + "   D    " + list[1] + " Register to system"+"\n" )
                    fo.close()
                    mutex.release()
                    ...
                    time.sleep(RandomIndex/1000)
                    result="D : " + "registration completed"
                    self.conn.send(result)
                ...
                mutex.acquire()
                fo = open("LogD.txt", "a")
                fo.write(datetime.date.today().ctime() + "   D    " +"Client request: "+ data+"\n" )
                fo.close()
                mutex.release()
                #end
                RandomIndex = randint(0, 1500)
...
TCP_IP = '0.0.0.0'
TCP_PORT = 9500 # listen to port 9500
BUFFER_SIZE = 2000
